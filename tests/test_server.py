...
class TestFindEndpointOptions:
    ...
    def test_find_with_allow_overlaps(self, client):
        """Test find with allow_overlaps option."""
        response = client.post(
            "/find",
            json={
    ...
    def test_find_with_include_matched_text(self, client):
        """Test find with include_matched_text option."""
        response = client.post(
            "/find",
            json={
                "text": "Visit https://example.com",
                "namespaces": ["comm"],
                "options": {"include_matched_text": True},
            },
        )
        assert response.status_code == 200
        data = response.json()
        # URL pattern has store_raw=True, so it will include matched text
        if data["count"] > 0:
            # Check that match field exists (might be None if policy doesn't allow)
            assert "match" in data["hits"][0]


class TestRedactEndpointOptions:
    """Tests for /redact endpoint with different strategies."""

    def test_redact_default_strategy(self, client):
        """Test redact with default strategy (None)."""
        response = client.post(
            "/redact",
            json={
                "text": "Email: test@example.com",
