...
docker-build:  ## Build Docker image
	docker build -t data-detector:latest -f docker/Dockerfile .

docker-run:  ## Run Docker container
	docker run -p 8080:8080 -p 9090:9090 data-detector:latest
...
serve:  ## Start development server
	data-detector serve --config config.yml --reload
