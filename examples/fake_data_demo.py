...
def demo_file_generation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print("     Contains: 100 rows with name, email, phone, SSN, credit card, etc.")
        ...
        print("     Contains: 50 user records with structured data")
        ...
        print("     Contains: 200 records in 'users' table")
        ...
        print("     Contains: 50 user records in XML format")

        # Log files
        print("\n5. Creating log files...")
        for log_format in ["apache", "json", "syslog"]:
            log_path = tmpdir / f"application.{log_format}.log"
            generator.create_log_file(log_path, lines=1000, include_pii=True, log_format=log_format)
        ...
        print("     Contains: 20 paragraphs with embedded PII")
...
def demo_office_files():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print("     Contains: 10 paragraphs with tables of PII data")
            ...
            print(f"   ✓ Created: {excel_path.name} ({excel_path.stat().st_size:,} bytes)")
            print("     Contains: 500 rows with styled headers and PII columns")
            ...
            print("     Contains: Title slide + 10 content slides with PII")
            ...
            try:
                ...
                print("     Contains: 5 pages with tables and PII")
                ...
                print("     Contains: Professional invoice with line items and PII")
            except ImportError:
...
def demo_image_generation():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print("     Size: 800x600, Format: PNG")
            print("     Contains: Name, email, phone, SSN, credit card, etc.")
            ...
            print("     Size: 1200x800, looks like terminal/editor")
            print("     Contains: Username, email, API keys, passwords")
            ...
            print("     Size: 1920x1080, Format: JPEG")
...
def demo_generate_and_detect():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print("   ✓ Generated CSV with 10 rows")
        ...
        print(
            f"     - Phone numbers: {sum(1 for m in result.matches if m.category.value == 'phone')}"
        )
        print(f"     - SSNs: {sum(1 for m in result.matches if m.category.value == 'national_id')}")
        num_cc = sum(1 for m in result.matches if m.category.value == "credit_card")
        print(f"     - Credit cards: {num_cc}")
        num_ip = sum(1 for m in result.matches if m.category.value == "ip_address")
        print(f"     - IP addresses: {num_ip}")
